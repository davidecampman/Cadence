...
def get_data_dir(subdir: str | None = None) -> Path:
    ...
    Resolution order:
        1. AGENT_ONE_DATA_DIR environment variable (explicit override)
        2. Platform default:
            - macOS:   ~/Library/Application Support/agent-one
            - Windows: %LOCALAPPDATA%/agent-one
            - Linux:   $XDG_DATA_HOME/agent-one  (default: ~/.local/share/agent-one)
    ...
    """
    explicit = os.environ.get("AGENT_ONE_DATA_DIR")
    if explicit:
        base = Path(explicit)
    else:
